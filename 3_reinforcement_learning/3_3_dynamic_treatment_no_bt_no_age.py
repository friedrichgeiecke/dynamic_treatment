    ...
    while ee < EE:
        ...
        while terminal_flag == False:
            ...
            for bb in range(0, batch_size):
                ...
                s0_prime = (
                    ...
                    .loc[sample_person_index, ["education", "prev_earnings"]]
                    .values.reshape(2, 1)
                )
        ...
        if process_number == 1:
            ...
            if ee == 1:
                ...
                value_file = open(value_file_path, "w")
            ...
            for evaluation_episode in range(0, evaluation_episodes):
                ...
                while terminal_flag == False:
                    ...
                    s0_prime = (
                        cluster_dict[cluster]
                        .loc[
                            sample_person_index, ["education", "prev_earnings"],
                        ]
                        .values.reshape(2, 1)
                    )
...
if __name__ == "__main__":
    ...
    df = df[["education", "prev_earnings", reward_column_name, "clstrs9"]]
    df = df.dropna()

    # Scale covariates
    scaler = StandardScaler()
    scaler.fit(df.loc[:, ["education", "prev_earnings"]])
    df.loc[:, ["education", "prev_earnings"]] = scaler.transform(
        df.loc[:, ["education", "prev_earnings"]]
    )
    ...
    for cc in range(0, 4):
        cluster_dict[cc] = df.loc[
            df.loc[:, "clstrs9"] == (cc + 1),
            ["education", "prev_earnings", reward_column_name],
        ]
